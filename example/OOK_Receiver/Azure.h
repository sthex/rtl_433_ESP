...
static const char* ssid = HEX_WIFI_IDM; //IOT_CONFIG_WIFI_SSID;
static const char* password = HEX_WIFI_passwordM; //IOT_CONFIG_WIFI_PASSWORD;
static const char* host = IOT_CONFIG_IOTHUB_FQDN;
static const char* mqtt_broker_uri = "mqtts://" IOT_CONFIG_IOTHUB_FQDN;
static const char* device_id = "rtl433wind"; //IOT_CONFIG_DEVICE_ID;
static const int mqtt_port = AZ_IOT_DEFAULT_MQTT_CONNECT_PORT;
//static const char* connectionString = CONNECTION_STRING_AZURE_rtl433wind;
...
static unsigned long max_next_telemetry_send_time_ms = 0;
static unsigned long min_next_telemetry_send_time_ms = 0;
...
static void generateIdleTelemetryPayload() {
  ...
  telemetry_payload = "{ \"deviceId\": \"" + String(IOT_CONFIG_DEVICE_ID) + "\", \"msgCount\": " + String(telemetry_send_count++) + " }";
}

static void send_azure() {
  if (millis() < min_next_telemetry_send_time_ms) {
    Logger.Info("Skip sending rapid telemetry.");
    return;
  }
  ...
  min_next_telemetry_send_time_ms = millis() + FASTEST_TELEMETRY_FREQUENCY_MILLISECS;
}
...
#endif
  else if (millis() > max_next_telemetry_send_time_ms) {
    generateIdleTelemetryPayload();
    send_azure();
    max_next_telemetry_send_time_ms = millis() + SLOWEST_TELEMETRY_FREQUENCY_MILLISECS;
  }
}